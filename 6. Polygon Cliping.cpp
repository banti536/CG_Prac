...
int leftclip(int a[10][2], int v, int wxmin) {
    ...
    for (i = 0; i < v; i++) {
        ...
        if ((a[i][0] > wxmin && a[(i + 1) % v][0] < wxmin) ||
            (a[i][0] < wxmin && a[(i + 1) % v][0] > wxmin)) {
            x1 = a[i][0]; y1 = a[i][1];
            x2 = a[(i + 1) % v][0]; y2 = a[(i + 1) % v][1];
...
int rightclip(int a[10][2], int v, int wxmax) {
    int i, t[10][2];
    int k = 0;
    int x1, y1, x2, y2;
    ...
    for (i = 0; i < v; i++) {
        ...
        if ((a[i][0] < wxmax && a[(i + 1) % v][0] > wxmax) ||
            (a[i][0] > wxmax && a[(i + 1) % v][0] < wxmax)) {
            x1 = a[i][0]; y1 = a[i][1];
            x2 = a[(i + 1) % v][0]; y2 = a[(i + 1) % v][1];
...
int topclip(int a[10][2], int v, int wymax) {
    ...
    for (i = 0; i < v; i++) {
        ...
        if ((a[i][1] < wymax && a[(i + 1) % v][1] > wymax) ||
            (a[i][1] > wymax && a[(i + 1) % v][1] < wymax)) {
            x1 = a[i][0]; y1 = a[i][1];
            x2 = a[(i + 1) % v][0]; y2 = a[(i + 1) % v][1];
            t[k][1] = wymax;
...
int bottomclip(int a[10][2], int v, int wymin) {
    ...
    for (i = 0; i < v; i++) {
        ...
        if ((a[i][1] > wymin && a[(i + 1) % v][1] < wymin) ||
            (a[i][1] < wymin && a[(i + 1) % v][1] > wymin)) {
            x1 = a[i][0]; y1 = a[i][1];
            x2 = a[(i + 1) % v][0]; y2 = a[(i + 1) % v][1];
            t[k][1] = wymin;
...
int main(int argc, char** argv) {
    ...
    do {
        ...
        glutPostRedisplay(); // Redisplay after clipping
